...
# Convert to DataFrame and coerce numerical columns safely
...
if not df.empty:
    ...
    # numeric conversions
    numeric_cols = ["qty","avg_price","current_price","value","unrealized_profit"]
        ...
        with st.expander("⚙️ Manage Holdings"):
            ...
            with c1:
                if st.button("💾 Save Changes"):
                    payload = {"symbol": symbol, "qty": float(new_qty), "price": float(new_price), "user_id": st.session_state.user_id}
            ...
            with c2:
                if st.button("🗑️ Delete Holding"):
                    try:
                        r = requests.delete(f"{BACKEND_URL}/holding/{symbol}", params={"user_id": st.session_state.user_id}, timeout=10)
                        if r.status_code == 200:
    ...
    with st.expander("➕ Add a Holding"):
        ...
        if submitted:
            if not symbol_in:
                ...
                payload = {"symbol": symbol_in, "qty": float(qty_in), "price": float(price_in), "user_id": st.session_state.user_id}
                try:
                    r = api_post("/holding", payload, timeout=10)
                    if r.status_code == 200:
                        st.success(f"✅ Holding saved: {symbol_in} ({qty_in} @ {price_in:.2f}). It may take a moment to reflect.")
                        time.sleep(0.6)
                        st.rerun()
                    else:
...
with right:
    with st.expander("💵 Sell a Holding"):
        # we need current holdings to offer symbols
